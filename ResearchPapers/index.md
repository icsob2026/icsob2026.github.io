---
layout: generic-page
title: Call for Papers
description: "Submit to ICSOB 2026. Topics include AI-native software business, digital sovereignty, cloud platforms, ecosystems, and sustainability. Submission deadline: July 18, 2026."
---

<h1 id="CallForResearchPapers" class="mb-4 text-center">Call for Papers</h1>

<h3 class="text-center mb-3"><i>"Software Business in the Age of AI-Native Dominance and Digital Sovereignty"</i></h3>

<p>Software business is entering a new phase shaped by profound technological, economic, societal, and geopolitical change. AI-native products and services are transforming how software is designed, operated, and monetized. At the same time, concerns about digital sovereignty, strategic autonomy, cloud dependence, semiconductor supply chains, infrastructure control, and energy availability are reshaping the conditions under which software businesses compete and grow.</p>

<p>The theme of ICSOB 2026, <strong>Software Business in the Age of AI-Native Dominance and Digital Sovereignty</strong>, invites the community to explore how software-intensive firms create value, remain resilient, and sustain legitimacy in a world increasingly influenced by AI, infrastructure concentration, geopolitical tensions, and the politics of digital technology.</p>

<p>We particularly invite work that examines questions such as:</p>

<ul>
  <li>How are software businesses reshaped by AI-native products, platform concentration, and infrastructure dependence?</li>
  <li>How do firms manage dependence on cloud providers, chips, energy systems, standards, and regulatory regimes?</li>
  <li>How do geopolitics, industrial policy, procurement, and political influence affect software markets and digital ecosystems?</li>
  <li>How should software organisations balance growth, speed, sustainability, inclusion, and trust?</li>
  <li>How are traditional software business domains such as product management, cloud services, digital transformation, and ecosystem strategy evolving in parallel with the AI wave?</li>
  <li>What new opportunities arise from cloud-native computing, edge infrastructures, 6G, and next-generation digital services?</li>
</ul>

<p>At the same time, ICSOB 2026 continues its long-standing mission as a broad forum for <strong>software business research and practice</strong>, explicitly welcoming contributions across the wide spectrum of topics that have traditionally defined the conference, including software product management, software ecosystems, cloud services, digital transformation, sustainability, ethics, inclusion, entrepreneurship, and emerging communications and infrastructure technologies.</p>

<p>ICSOB 2026 further invites rigorous and relevant work from a range of disciplines, such as software engineering, information systems, innovation and entrepreneurship, strategic management, economics, digital policy, design science, and related fields.</p>

<hr>

<h2>Topics of Interest</h2>

<p>Topics include, but are not limited to, the following.</p>

<h3>A. Annual theme: AI-native dominance, digital sovereignty, and strategic dependence</h3>
<ul>
  <li>AI-native software businesses, products, and business models</li>
  <li>digital sovereignty, strategic autonomy, and sovereign digital infrastructures</li>
  <li>dependence on cloud hyperscalers, model providers, and platform gatekeepers</li>
  <li>geopolitical risk, industrial policy, export controls, and market fragmentation</li>
  <li>semiconductors, accelerators, chips, and hardware-software co-dependence</li>
  <li>electricity, data centers, energy intensity, and infrastructure location strategies</li>
  <li>critical infrastructure, cyber resilience, and trusted software-intensive systems</li>
  <li>political influence, lobbying, standard-setting, procurement, and regulatory power</li>
  <li>value capture, lock-in, interoperability, and ecosystem control in AI-intensive markets</li>
</ul>

<h3>B. Software product management and software-intensive innovation</h3>
<ul>
  <li>software product strategy, roadmapping, and portfolio management</li>
  <li>product discovery, experimentation, and continuous value delivery</li>
  <li>product-market fit, software pricing, and monetization</li>
  <li>requirements engineering and decision-making in software product contexts</li>
  <li>software quality, technical debt, and lifecycle management</li>
  <li>DevOps, platform engineering, and product operations</li>
  <li>software-enabled service innovation and digital product innovation</li>
</ul>

<h3>C. Cloud computing, platforms, and next-generation infrastructures</h3>
<ul>
  <li>cloud computing business models and cloud service ecosystems</li>
  <li>multi-cloud, hybrid cloud, edge-cloud, and distributed computing strategies</li>
  <li>platform business, platform governance, and API ecosystems</li>
  <li>software as a service, platform as a service, and infrastructure as a service</li>
  <li>economics and management of cloud-native software businesses</li>
  <li>edge computing, Internet of Things, and cyber-physical service systems</li>
  <li>future connectivity, and software-enabled communication services</li>
  <li>digital infrastructures for smart cities, mobility, health, manufacturing, and public services</li>
</ul>

<h3>D. Software ecosystems, open innovation, and digital platforms</h3>
<ul>
  <li>software ecosystems, ecosystem governance, and orchestration</li>
  <li>complementors, boundary resources, and platform openness</li>
  <li>open source, open-core, and ecosystem-based business strategies</li>
  <li>coopetition, partner strategies, and interorganizational value creation</li>
  <li>app stores, marketplaces, and digital platform competition</li>
  <li>standards, interoperability, and modularity in software-intensive ecosystems</li>
</ul>

<h3>E. Sustainability, ethics, inclusion, and responsible software business</h3>
<ul>
  <li>sustainability in software business, digital services, and ecosystems</li>
  <li>green software, energy-efficient architectures, and responsible infrastructure use</li>
  <li>environmental, social, and economic sustainability of software-intensive business</li>
  <li>ethics in software business, software development, and digital services</li>
  <li>fairness, accountability, transparency, and responsible governance</li>
  <li>diversity, inclusion, accessibility, and participation in digital innovation</li>
  <li>digital rights, user autonomy, and socially responsible software business</li>
  <li>societal impacts of software platforms and digital transformation</li>
</ul>

<h3>F. Entrepreneurship, startups, and software business development</h3>
<ul>
  <li>software startups, scale-ups, and entrepreneurial software ventures</li>
  <li>venture creation, business experimentation, and growth strategies</li>
  <li>software business models and internationalization</li>
  <li>software-intensive innovation management and commercialization</li>
  <li>startup ecosystems, accelerators, and venture financing</li>
  <li>AI-native startups and emerging digital opportunities</li>
</ul>

<h3>G. Digital transformation, services, and organizational change</h3>
<ul>
  <li>digital transformation through software and digital services</li>
  <li>transformation of incumbents into software-centric businesses</li>
  <li>service-dominant logic and software-enabled service business</li>
  <li>organizational capabilities, talent, and leadership in software-intensive firms</li>
  <li>data-driven business, operational transformation, and digital operating models</li>
  <li>public sector digitalization and software-enabled societal transformation</li>
</ul>

<h3>H. Governance, regulation, and public value</h3>
<ul>
  <li>governance of software-intensive business and digital markets</li>
  <li>regulation of platforms, cloud services, and digital ecosystems</li>
  <li>public procurement, compliance, and certification</li>
  <li>privacy, cybersecurity, trust, and software supply chain governance</li>
  <li>software business in regulated industries and critical sectors</li>
  <li>law, policy, and institutional change in digital societies</li>
</ul>

<hr>

<h2>Important Dates</h2>

{% include important-dates/_research-papers.html %}

<hr>

<h2>Submission Guidelines</h2>
{% include submission-guidelines/_research-papers.html %}